...
if (target != undefined)
{
    var col = collision_line(x, y, (x + sign(image_xscale) * 180), y, target, false, false)
    if col
    {
        if (burst_timer > 0)
            burst_timer--
...
else
{
    ...
    if (vspd < -0.6)
        ...
        iindex = 2
    if (vspd > 0.5 && y > (target_y + 24))
    {
        vspd = (-jump_height)
        var _pd = point_direction(x, y, target_x, target_y)
        hspd = lengthdir_x(4, _pd)
        state = 654
    }
}
gary_collisions()
...
if (col && state == 657 && burst_timer && burst_timer < 16)
{
